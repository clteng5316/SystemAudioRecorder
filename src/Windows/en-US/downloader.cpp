...
static BOOL ensure_output_directory(DOWNLOADER_STATE *state)
{
    ...
    if (error == ERROR_ALREADY_EXISTS && directory_exists(directory))
        return TRUE;
    set_message(state, L"The download directory does not exist and could not be created.");
    return FALSE;
};
...
static DWORD WINAPI downloader_thread(LPVOID parameter)
{
    ...
    {
        set_message(state, L"Unable to determine the FFmpeg directory.");
    ...
    if (FAILED(hr))
    {
        set_message(state, L"The download command is too long.");
    ...
    if (!CreatePipe(&readPipe, &writePipe, &security, 0))
    {
        ...
        set_message(state, L"Unable to create the downloader output pipe.");
        goto Finished;
    };
    ...
    set_message(state, mode == DOWNLOAD_MODE_AUDIO ? L"Starting audio download..." : L"Starting video download...");
    ...
    if (!created)
    {
        WCHAR errorText[256];
        exitCode = GetLastError();
        StringCchPrintfW(errorText, ARRAY_COUNT(errorText), L"Unable to start yt-dlp.exe (Windows error %lu).", exitCode);
    ...
    if (exitCode == 0)
    {
        ...
        StringCchPrintfW(state->message, ARRAY_COUNT(state->message), L"Download complete. Saved to: %s", state->outputDirectory);
    }
    else
    {
        WCHAR prior[DOWNLOADER_TEXT_MAX];
        StringCchCopyW(prior, ARRAY_COUNT(prior), state->message);
        StringCchPrintfW(state->message, ARRAY_COUNT(state->message), L"Download failed (yt-dlp exit code %lu). %s", exitCode, prior);
    };
    state->busy = FALSE;
    LeaveCriticalSection(&state->lock);
    return 0;
...
BOOL DownloaderSetOutputDirectory(DOWNLOADER_STATE *state, LPCWSTR directory)
{
    ...
    if (!directory_exists(directory))
    {
        set_message(state, L"The selected download folder does not exist or is not a valid directory.");
        return FALSE;
    };
    EnterCriticalSection(&state->lock);
    if (state->busy)
    {
        StringCchCopyW(state->message, ARRAY_COUNT(state->message), L"The download folder cannot be changed while a download is running.");
        LeaveCriticalSection(&state->lock);
        return FALSE;
    };
    ...
    StringCchCopyW(state->message, ARRAY_COUNT(state->message), L"Download folder changed.");
...
BOOL DownloaderInitialize(DOWNLOADER_STATE *state)
{
    ...
    {
        set_message(state, L"Unable to determine the application directory.");
        return FALSE;
    };
    if (FAILED(StringCchPrintfW(state->outputDirectory, ARRAY_COUNT(state->outputDirectory), L"%s\\downloads", state->exeDirectory)))
    {
        set_message(state, L"The download directory path is too long.");
        return FALSE;
    };
    DownloaderRefreshTools(state);
    set_message(state, L"URL downloader ready. External yt-dlp.exe and ffmpeg.exe are optional helpers.");
    return TRUE;
};
...
BOOL DownloaderStart(DOWNLOADER_STATE *state, LPCWSTR url, int mode)
{
    ...
    {
        if (state != NULL && state->lockInitialized)
            set_message(state, L"Enter a valid http:// or https:// URL without quotes or line breaks.");
        return FALSE;
    };
    ...
    {
        LeaveCriticalSection(&state->lock);
        set_message(state, L"A download is already running.");
        return FALSE;
    };
    if (state->ytDlpPath[0] == L'\0')
    {
        LeaveCriticalSection(&state->lock);
        set_message(state, L"yt-dlp.exe was not found. Put it in the tools folder beside SystemAudioRecorder.exe.");
        return FALSE;
    };
    if (state->ffmpegPath[0] == L'\0')
    {
        LeaveCriticalSection(&state->lock);
        set_message(state, L"ffmpeg.exe was not found. Put it in the tools folder beside SystemAudioRecorder.exe.");
        return FALSE;
    }
    ...
    StringCchCopyW(state->message, ARRAY_COUNT(state->message), mode == DOWNLOAD_MODE_AUDIO ? L"Preparing audio download..." : L"Preparing video download...");
    LeaveCriticalSection(&state->lock);
    ...
    if (thread == NULL)
    {
        ...
        state->busy    = FALSE;
        state->success = FALSE;
        StringCchCopyW(state->message, ARRAY_COUNT(state->message), L"Unable to create the download worker thread.");
        LeaveCriticalSection(&state->lock);
        return FALSE;
    };
    EnterCriticalSection(&state->lock);
