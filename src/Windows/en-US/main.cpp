...
static void draw_spectrum(HDC dc, const RECT *client)
{
    ...
    top.bottom = top.top + 56;
    {
        ...
        StringCchPrintfW(text, ARRAY_COUNT(text), L"Peak %.0f Hz", snap.dominantHz);
        draw_text_center(dc, &left, text, g_bigFont, RGB(75, 220, 245));
        StringCchPrintfW(text, ARRAY_COUNT(text), status.recording ? L"MP3 Recording | 192 kbps" : L"MP3 Ready");
    ...
    };
    {
        ...
        StringCchPrintfW(text, ARRAY_COUNT(text), L"RMS %.1f dBFS    %lu Hz / %u ch / %u bit%s    FFT 4096 + Hann + 75%% overlap",
                         snap.rmsDb, status.sampleRate, status.channels, status.bitsPerSample, status.isFloat ? L" float" : L"");
...
static void update_controls(void)
{
    ...
    if (audioStatus.converting)
        StringCchPrintfW(audioText, ARRAY_COUNT(audioText), L"MP3 conversion | %s", audioStatus.recorderMessage);
    else if (audioStatus.recording)
    {
        StringCchPrintfW(audioText, ARRAY_COUNT(audioText), L"MP3 recording: %s | Gain: %s",
                                                            audioStatus.recordPath, record_gain_label(selected_record_gain_mode()));
    }
    else if (audioStatus.running)
    {
        StringCchPrintfW(audioText, ARRAY_COUNT(audioText), L"Capturing system audio | %lu Hz, %u channels | %s",
                                                            audioStatus.sampleRate, audioStatus.channels, audioStatus.lastError);
    }
    else
    {
        StringCchPrintfW(audioText, ARRAY_COUNT(audioText), L"Stopped | %s", audioStatus.lastError);
    };
    if (active > 0 || waiting > 0)
        StringCchPrintfW(text, ARRAY_COUNT(text), L"Download queue: %d active / %d waiting | %s", active, waiting, audioText);
    else
...
static BOOL choose_mp3_path(HWND hwnd, WCHAR *path, DWORD count)
{
    ...
    ofn.hwndOwner   = hwnd;
    ofn.lpstrFilter = L"MP3 audio (*.mp3)\0*.mp3\0All files (*.*)\0*.*\0\0";
    ofn.lpstrFile   = path;
    ofn.nMaxFile    = count;
...
static LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_CREATE:
            g_start     = CreateWindowW(L"BUTTON", L"Start Capture", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_START, GetModuleHandleW(NULL), NULL);
            g_stop      = CreateWindowW(L"BUTTON", L"Stop", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_STOP, GetModuleHandleW(NULL), NULL);
            g_rec       = CreateWindowW(L"BUTTON", L"Record MP3", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_REC, GetModuleHandleW(NULL), NULL);
            g_stopRec   = CreateWindowW(L"BUTTON", L"Stop MP3", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_STOPREC, GetModuleHandleW(NULL), NULL);
            g_gainLabel = CreateWindowW(L"STATIC", L"Rec gain:", WS_CHILD | WS_VISIBLE | SS_LEFT, 0,0,0,0, hwnd, NULL, GetModuleHandleW(NULL), NULL);
            ...
            g_download  = CreateWindowW(L"BUTTON", L"Downloads...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_DOWNLOADQ, GetModuleHandleW(NULL), NULL);
            g_about     = CreateWindowW(L"BUTTON", L"About", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0,0,0,0, hwnd, (HMENU)IDC_ABOUT, GetModuleHandleW(NULL), NULL);
            g_status    = CreateWindowW(L"STATIC", L"Ready", WS_CHILD | WS_VISIBLE | SS_LEFT | SS_ENDELLIPSIS, 0,0,0,0, hwnd, (HMENU)IDC_STATUS, GetModuleHandleW(NULL), NULL);
            if (g_gainCombo != NULL)
            {
                LRESULT item;
                ...
                item = SendMessageW(g_gainCombo, CB_ADDSTRING, 0, (LPARAM)L"+6 dB (Recommended)");
                ...
                item = SendMessageW(g_gainCombo, CB_ADDSTRING, 0, (LPARAM)L"Auto Normalize");
            ...
            switch (LOWORD(wParam))
            {
                case IDC_START:
                    if (!AudioEngineStart(&g_engine))
                    {
                        ...
                        MessageBoxW(hwnd, st.lastError, L"Audio Capture Error", MB_OK | MB_ICONERROR);
                    };
                    update_controls();
                    return 0;
                case IDC_STOP:
                ...
                case IDC_REC:
                {
                    ...
                    if (choose_mp3_path(hwnd, path, (DWORD)ARRAY_COUNT(path)))
                    {
                        if (!AudioEngineStartMp3(&g_engine, path, 192, selected_record_gain_mode()))
                        {
                            AUDIO_ENGINE_STATUS st;
                            AudioEngineGetStatus(&g_engine, &st);
                            MessageBoxW(hwnd, st.lastError, L"MP3 Conversion", MB_OK | MB_ICONERROR);
                        };
                        update_controls();
                    };
                };
                ...
                case IDC_ABOUT:
                    MessageBoxW(hwnd, L"SystemAudioRecorder v2.0-beta.1\r\n"
                                      ...
                                      L"System audio: Windows output capture (WASAPI loopback backend).\r\n"
                                      L"Spectrum: 4096-point FFT, Hann, 75% overlap.\r\n"
                                      L"MP3: selectable gain/normalization via standalone/static ffmpeg.exe.\r\n"
                                      L"Downloads: one-item-at-a-time FIFO queue using yt-dlp + ffmpeg.\r\n"
                                      L"Queued items remain in memory until SystemAudioRecorder exits.",
                                      L"About SystemAudioRecorder", MB_OK | MB_ICONINFORMATION);
                    return 0;
            };
            break;
        case WM_ERASEBKGND:
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nShowCmd)
{
    ...
    {
        MessageBoxW(NULL, L"Audio engine initialization failed.",  L"SystemAudioRecorder", MB_OK | MB_ICONERROR);
        return 1;
    };
    if (!DownloaderInitialize(&g_downloader))
    {
        MessageBoxW(NULL, L"Downloader initialization failed.", L"SystemAudioRecorder", MB_OK | MB_ICONERROR);
        DownloaderDestroy(&g_downloader);
        AudioEngineDestroy(&g_engine);
        return 1;
    };
    ...
    wc.lpszClassName = L"SystemAudioRecorderV20Beta1EN";
    if (!RegisterClassExW(&wc))
        return 1;
    hwnd = CreateWindowExW(0, wc.lpszClassName, L"SystemAudioRecorder v2.0-beta.1 - System Audio Spectrum",
