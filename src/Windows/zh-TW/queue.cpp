...
static BOOL confirm_notice(HWND owner)
{
    ...
    answer = MessageBoxW(owner,
                         L"URL 下載使用外部 yt-dlp 與 FFmpeg。僅在法律及來源平台／服務條款允許此下載方式時使用。本程式不提供 DRM、付費內容、Cookie、登入或存取控制繞過。是否繼續？",
                         L"URL 下載提醒",
...
static void show_url_menu(HWND edit, LPARAM lParam)
{
    ...
    AppendMenuW(menu, selectedFlags, DQ_MENU_CUT, L"剪下");
    AppendMenuW(menu, selectedFlags, DQ_MENU_COPY, L"複製");
    AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
    AppendMenuW(menu, clipboard_has_text() ? MF_STRING : (MF_STRING | MF_GRAYED),
                DQ_MENU_PASTE, L"貼上");
    AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
    AppendMenuW(menu, MF_STRING, DQ_MENU_ALL, L"全選");
...
static void refresh_list(void)
{
    ...
    for (i = 0; i < g_count; ++i)
    {
        ...
        else if (g_items[i].state == DQ_STATE_ACTIVE) nameText = L"正在取得名稱...";
        else if (g_items[i].state == DQ_STATE_WAITING) nameText = L"等待取得名稱";
        else nameText = L"-";
        list_set_text(i, 1, nameText);
        list_set_text(i, 2, g_items[i].url);

        typeText = (g_items[i].mode == DOWNLOAD_MODE_AUDIO) ? L"音訊" : L"影片";
        list_set_text(i, 3, typeText);

        if (g_items[i].state == DQ_STATE_WAITING)
            StringCchCopyW(progress, ARRAY_COUNT(progress), L"等待中");
            ...
            StringCchCopyW(progress, ARRAY_COUNT(progress), L"已取消");
        else
            StringCchCopyW(progress, ARRAY_COUNT(progress), L"失敗");
        list_set_text(i, 4, progress);

        if (g_items[i].state == DQ_STATE_DONE) doneText = L"是";
        else if (g_items[i].state == DQ_STATE_FAILED) doneText = L"失敗";
        else if (g_items[i].state == DQ_STATE_CANCELED) doneText = L"已取消";
        else doneText = L"否";
                               ...
                               int canceledCount, int failedCount)
{
    ...
    nid.uID = DQ_TRAY_ID;
    nid.uFlags = NIF_INFO;
    StringCchCopyW(nid.szInfoTitle, ARRAY_COUNT(nid.szInfoTitle), L"SystemAudioRecorder 下載完成");
    if (canceledCount > 0 || failedCount > 0)
        StringCchPrintfW(info, ARRAY_COUNT(info), L"佇列完成：%d 個影片、%d 個音訊；%d 個取消、%d 個失敗。",
                         videos, audioCount, canceledCount, failedCount);
    else
        StringCchPrintfW(info, ARRAY_COUNT(info), L"佇列完成：%d 個影片、%d 個音訊。", videos, audioCount);
...
static void start_next(HWND notifyOwner)
{
    ...
    for (;;)
    {
        ...
        g_active = -1;
        refresh_list();
        MessageBoxW(notifyOwner, status.message, L"無法開始下載", MB_OK | MB_ICONERROR);
    }
}
...
static void browse_folder(HWND owner)
{
    ...
    if (g_active >= 0)
    {
        MessageBoxW(owner, L"下載正在進行中，請等目前項目完成後再變更資料夾。", L"儲存至：", MB_OK | MB_ICONINFORMATION);
        return;
    }
    ...
    if (SHGetPathFromIDListW(item, selected))
    {
        ...
        {
            DownloaderGetStatus(g_downloader, &status);
            MessageBoxW(owner, status.message, L"儲存至：", MB_OK | MB_ICONERROR);
        }
    }
    CoTaskMemFree(item);
}
...
static void add_item(HWND owner)
{
    ...
    {
        MessageBoxW(owner, L"下載佇列已滿（最多 128 個項目）。", L"下載佇列", MB_OK | MB_ICONINFORMATION);
        return;
    }
    ...
    {
        MessageBoxW(owner, L"請輸入有效的 http:// 或 https:// URL。", L"下載佇列", MB_OK | MB_ICONINFORMATION);
        return;
    }
...
static void add_columns(void)
{
    ...
    column.iSubItem = 1; column.cx = 250; column.pszText = L"名稱";
    ...
    column.iSubItem = 3; column.cx = 72; column.pszText = L"類型";
    ListView_InsertColumn(g_list, 3, &column);
    column.iSubItem = 4; column.cx = 78; column.pszText = L"進度";
    ListView_InsertColumn(g_list, 4, &column);
    column.iSubItem = 5; column.cx = 72; column.pszText = L"完成";
    ...
    case WM_CREATE:
        {
            ...
            g_typeLabel = CreateWindowW(L"STATIC", L"類型：",
                                     WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE,
                                     0, 0, 0, 0, hwnd, NULL, g_instance, NULL);
            g_type = CreateWindowW(L"COMBOBOX", L"",
                                   ...
                                   g_instance, NULL);
            SendMessageW(g_type, CB_ADDSTRING, 0, (LPARAM)L"影片");
            SendMessageW(g_type, CB_ADDSTRING, 0, (LPARAM)L"音訊");
            SendMessageW(g_type, CB_SETCURSEL, 0, 0);

            g_add = CreateWindowW(L"BUTTON", L"加入佇列",
                                  WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                                  0, 0, 0, 0, hwnd, (HMENU)DQ_ID_ADD,
                                  g_instance, NULL);

            g_folderLabel = CreateWindowW(L"STATIC", L"儲存至：",
                                        WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE,
                                        0, 0, 0, 0, hwnd, NULL, g_instance, NULL);
            g_folder = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
                                       ...
                                       g_instance, NULL);
            g_browse = CreateWindowW(L"BUTTON", L"瀏覽...",
                                     WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
...
BOOL DownloadQueueInitialize(HINSTANCE instance, DOWNLOADER_STATE *downloader)
{
    ...
    wc.lpszClassName = L"SystemAudioRecorderDownloadQueueV20Beta1ZHTW";
...
void DownloadQueueShow(HWND owner)
{
    ...
    if (g_window == NULL)
    {
        g_window = CreateWindowExW(WS_EX_APPWINDOW,
                                   L"SystemAudioRecorderDownloadQueueV20Beta1ZHTW",
                                   L"SystemAudioRecorder - 下載佇列",
        ...
        else if (status.exitCode != STILL_ACTIVE)
        {
            ...
            if (!succeeded)
                MessageBoxW(g_window != NULL ? g_window : owner,
                            status.message, L"下載失敗", MB_OK | MB_ICONERROR);
