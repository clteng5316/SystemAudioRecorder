...
static void set_status_error(AUDIO_ENGINE *engine, LPCWSTR text)
{
    ...
    StringCchCopyW(engine->status.lastError, ARRAY_COUNT(engine->status.lastError), text != NULL ? text : L"未知音訊錯誤");
...
static DWORD WINAPI capture_thread(LPVOID context)
{
    ...
    if (FAILED(hr))
    {
        set_status_error(engine, L"CoInitializeEx 初始化失敗。");
        goto Exit;
    };
    init = TRUE;
    ...
    if (FAILED(hr))
    {
        set_status_error(engine, L"無法建立 MMDeviceEnumerator。");
        goto Exit;
    };
    hr = enumerator->GetDefaultAudioEndpoint(eRender, eConsole, &device);
    if (FAILED(hr))
    {
        set_status_error(engine, L"找不到預設播放裝置。");
        goto Exit;
    };
    hr = device->Activate(__uuidof(IAudioClient), CLSCTX_ALL, NULL, (void **)&client);
    if (FAILED(hr))
    {
        set_status_error(engine, L"無法啟用 IAudioClient。");
        goto Exit;
    };
    hr = client->GetMixFormat(&wfx);
    if (FAILED(hr) || wfx == NULL)
    {
        set_status_error(engine, L"無法讀取播放裝置的混音格式。");
        goto Exit;
    };
    ...
    if (!isFloat && !isPcm)
    {
        set_status_error(engine, L"播放裝置使用不支援的取樣格式。");
        goto Exit;
    };
    ...
    if (FAILED(hr))
    {
        set_status_error(engine, L"WASAPI loopback 初始化失敗。");
        goto Exit;
    };
    hr = client->GetService(__uuidof(IAudioCaptureClient), (void **)&capture);
    if (FAILED(hr))
    {
        set_status_error(engine, L"無法取得 IAudioCaptureClient。");
        goto Exit;
    };
    ...
    if (avrtModule != NULL)
    {
        ...
        {
            mmcss = pAvSetMmThreadCharacteristicsW(L"音訊", &taskIndex);
        };
    };
    hr = client->Start();
    if (FAILED(hr))
    {
        set_status_error(engine, L"無法啟動 WASAPI loopback 擷取。");
        goto Exit;
    };
    started = TRUE;
    EnterCriticalSection(&engine->lock);
    ...
    while (InterlockedCompareExchange(&engine->stopRequested, 0, 0) == 0)
    {
        ...
        if (FAILED(hr))
        {
            set_status_error(engine, L"GetNextPacketSize 失敗。");
            break;
        };
        while (frame != 0 && InterlockedCompareExchange(&engine->stopRequested, 0, 0) == 0)
        {
            ...
            if (FAILED(hr))
            {
                set_status_error(engine, L"IAudioCaptureClient::GetBuffer 失敗。");
                break;
            };
            ...
            if (FAILED(hr))
            {
                set_status_error(engine, L"IAudioCaptureClient::ReleaseBuffer 失敗。");
                break;
            };
            hr = capture->GetNextPacketSize(&frame);
            if (FAILED(hr))
...
BOOL AudioEngineInitialize(AUDIO_ENGINE *engine)
{
    ...
    StringCchCopyW(engine->status.lastError, ARRAY_COUNT(engine->status.lastError), L"就緒");
    return TRUE;
};
...
BOOL AudioEngineStart(AUDIO_ENGINE *engine)
{
    ...
    if (engine->thread == NULL)
    {
        set_status_error(engine, L"無法建立音訊擷取執行緒。");
        return FALSE;
    };
    return TRUE;
};
...
BOOL AudioEngineStartMp3(AUDIO_ENGINE *engine, LPCWSTR path, LONG bitrateKbps, LONG gainMode)
{
    ...
    if (rate == 0)
    {
        set_status_error(engine, L"錄製 MP3 前請先啟動系統音訊擷取。");
        return FALSE;
    };
    EnterCriticalSection(&engine->recorderLock);
