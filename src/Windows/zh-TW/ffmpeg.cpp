...
static void set_message_locked(FFMPEG_MP3_WRITER *writer, LPCWSTR message)
{
    if (writer == NULL)
        return;
    StringCchCopyW(writer->message, ARRAY_COUNT(writer->message), message != NULL ? message : L"未知錄音錯誤");
};
...
static BOOL finalize_wave_locked(FFMPEG_MP3_WRITER *writer)
{
    ...
    {
        set_message_locked(writer, L"錄音已超過傳統 WAV 的 4 GB 上限。");
        return FALSE;
    };
    ...
    {
        set_message_locked(writer, L"無法定位到 WAV 標頭。");
        return FALSE;
    };
    if (!write_all(writer->waveFile, header, WAV_HEADER_BYTES))
    {
        set_message_locked(writer, L"無法完成 WAV 標頭。");
        return FALSE;
    };
...
static DWORD WINAPI convert_thread(LPVOID parameter)
{
    ...
    if (FAILED(hr))
    {
        ...
        set_message_locked(writer, L"FFmpeg MP3 轉換命令過長；暫存 WAV 已保留。");
        LeaveCriticalSection(&writer->lock);
    ...
    if (!created)
    {
        ...
        StringCchPrintfW(writer->message, ARRAY_COUNT(writer->message),
                         L"無法啟動 ffmpeg.exe（Windows 錯誤 %lu）。請使用獨立／靜態 FFmpeg；暫存 WAV 已保留。", error);
        LeaveCriticalSection(&writer->lock);
        return error;
    };
    ...
    if (exitCode == 0)
    {
        ...
        StringCchPrintfW(writer->message, ARRAY_COUNT(writer->message), L"MP3 轉換完成：%s", writer->targetPath);
    }
    else
    {
        StringCchPrintfW(writer->message, ARRAY_COUNT(writer->message),
                         L"FFmpeg MP3 轉換失敗（結束碼 %lu）。請使用支援 MP3 編碼的獨立／靜態 ffmpeg.exe。暫存 WAV 已保留：%s",
...
BOOL FfmpegMp3Initialize(FFMPEG_MP3_WRITER *writer)
{
    ...
    set_message_locked(writer, L"MP3 錄音器就緒；需要 tools\\ffmpeg.exe。");
    return TRUE;
};
...
BOOL FfmpegMp3Start(FFMPEG_MP3_WRITER *writer, LPCWSTR path, LONG sampleRate, LONG bitrateKbps, LONG gainMode)
{
    ...
    {
        EnterCriticalSection(&writer->lock);
        set_message_locked(writer, L"找不到 ffmpeg.exe。請將獨立／靜態版本放在 SystemAudioRecorder.exe 旁的 tools 資料夾。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    EnterCriticalSection(&writer->lock);
    ...
    if (writer->recording)
    {
        set_message_locked(writer, L"目前已有錄音正在進行。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    if (writer->converting || writer->convertThread != NULL)
    {
        set_message_locked(writer, L"上一段錄音仍在轉換成 MP3。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    ...
    if (FAILED(hr))
    {
        set_message_locked(writer, L"MP3 輸出路徑過長，無法建立暫存 WAV。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    ...
    if (writer->waveFile == INVALID_HANDLE_VALUE)
    {
        set_message_locked(writer, L"無法建立 MP3 錄音所需的暫存 WAV。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    ...
    if (!ok)
    {
        ...
        writer->waveFile = INVALID_HANDLE_VALUE;
        DeleteFileW(writer->temporaryWavePath);
        set_message_locked(writer, L"無法寫入暫存 WAV 標頭。");
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
    writer->recording = TRUE;
    set_message_locked(writer, L"正在將 PCM 錄到暫存 WAV；停止錄音後會以 ffmpeg.exe 轉成 MP3。");
    LeaveCriticalSection(&writer->lock);
    return TRUE;
};
BOOL FfmpegMp3WriteStereo16(FFMPEG_MP3_WRITER *writer, const short *samples, UINT frames)
{
    ...
    {
        set_message_locked(writer, L"錄音已達傳統 WAV 的 4 GB 上限。");
        ok = FALSE;
    }
    else
    {
        ...
        {
            set_message_locked(writer, L"寫入暫存 WAV 錄音失敗。");
        };
    };
    LeaveCriticalSection(&writer->lock);
    return ok;
...
BOOL FfmpegMp3Stop(FFMPEG_MP3_WRITER *writer)
{
    ...
    if (!finalized || writer->dataBytes == 0)
    {
        ...
        {
            set_message_locked(writer, L"沒有錄到音訊樣本，因此未啟動 MP3 轉換。");
            DeleteFileW(writer->temporaryWavePath);
        };
        writer->success = FALSE;
        writer->converting = FALSE;
    ...
    writer->success    = FALSE;
    writer->exitCode   = STILL_ACTIVE;
    set_message_locked(writer, L"錄音已停止，正在使用 ffmpeg.exe 將暫存 WAV 轉成 MP3...");
    ...
    if (thread == NULL)
    {
        ...
        StringCchPrintfW(writer->message, ARRAY_COUNT(writer->message),
                                          L"無法建立 FFmpeg 轉換執行緒（Windows 錯誤 %lu）。暫存 WAV 已保留。", writer->exitCode);
        LeaveCriticalSection(&writer->lock);
        return FALSE;
    };
