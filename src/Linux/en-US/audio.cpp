...
static void capture_loop(AudioEngine *engine) {
    ...
    if (source.empty()) { std::lock_guard<std::mutex> g(engine->lock); engine->status.message="Unable to find a PulseAudio/PipeWire monitor source; verify that pactl is available"; return; }
    int pipefd[2];
    if (pipe(pipefd)!=0) { std::lock_guard<std::mutex> g(engine->lock); engine->status.message="Unable to create the FFmpeg audio pipe"; return; }
    pid_t pid=fork();
    if (pid==0) {
    ...
    close(pipefd[1]);
    if (pid<0) { close(pipefd[0]); std::lock_guard<std::mutex> g(engine->lock); engine->status.message="Unable to start FFmpeg"; return; }
    engine->ffmpegPid=pid;
    {
        std::lock_guard<std::mutex> g(engine->lock); engine->status.running=true; engine->status.monitorSource=source; engine->status.message="Capturing system audio";
    }
    ...
    if (!engine->stopRequested.load()) engine->status.message="The audio capture process ended; check FFmpeg PulseAudio input support and the monitor source";
    else engine->status.message="Stopped";
}
